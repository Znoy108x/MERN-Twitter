    ...
    try {
      ...
      if (UserData) {
       const UpdatedUser =  await User.findByIdAndUpdate(
        ...
        return res.status(200).json({ success: true, message: "User Updated" , NewUser : UpdatedUser});
      }
  ...
  try {
    ...
    try {
      const a = await User.findByIdAndUpdate(
        { _id: from },
        { $set: {Following: removedFollowing } }  , {returnDocument: 'after'} 
      );
      const b = await User.findByIdAndUpdate(
        { _id: to },
        { $set: {Followers: removedFollower } }  , {returnDocument: 'after'}
      );
      return res.status(200).json({ success: true, message: "Unfollowed!" , From : a , To : b});
    } catch (err) {
